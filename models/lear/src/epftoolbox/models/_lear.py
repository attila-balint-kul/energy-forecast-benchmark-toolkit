...
class LEAR(object):
    ...
    def save_model(self, next_day_date):
        ...
        model_data = {'models': self.models, 'scalerX': self.scalerX, 'scalerY': self.scalerY}
    ...
    def _build_and_split_XYs(self, df_train, df_test=None, date_test=None):
        ...
        for hour in range(24):
            ...
            for past_day in [1, 2, 3, 7]:
                ...
                pastIndexTrain = pd.to_datetime(indexTrain.loc[:, 'h' + str(hour)].values) - pd.Timedelta(
                    hours=24 * past_day
                )
                pastIndexTest = pd.to_datetime(indexTest.loc[:, 'h' + str(hour)].values) - pd.Timedelta(
                    hours=24 * past_day
                )
    ...
    def recalibrate_and_forecast_next_day(self, df, calibration_window, next_day_date):
        ...
        (
            ...
            Ytrain,
            Xtest,
        ) = self._build_and_split_XYs(df_train=df_train, df_test=df_test, date_test=next_day_date)
...
def evaluate_lear_in_test_dataset(
    ...
    forecast_file_name = (
        'LEAR_forecast' + '_dat' + str(dataset) + '_YT' + str(years_test) + '_CW' + str(calibration_window) + '.csv'
    )
